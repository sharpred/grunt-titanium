...
module.exports = function(grunt) {
	...
	function execCommand(command, options, callback) {
		...
		grunt.log.writeln("length: " + extraArgs.length);
		extraArgs.unshift(command);
		grunt.log.writeln("length: " + extraArgs.length);
		extraArgs.unshift("ti");
		grunt.log.writeln("length: " + extraArgs.length);
		extraArgs = extraArgs.concat(args);
		grunt.log.writeln("length: " + extraArgs.length);
		//var tiarray = [command].concat(extraArgs.toString().split(","));
		//extraArgs.split(",").unshift(command);

		// add non-option, non-flag arguments
		//args = args.concat(command);
		//args.unshift(tiarray.split(","));
		//args.unshift("ti");
grunt.log.writeln("extraArgs: " + extraArgs);
		// spawn command and output
		grunt.log.writeln('appc ' + extraArgs.join(' '));
		var tiOpts = process.env.GRUNT_TITANIUM_TEST || success || failure ?
				{} : {stdio: 'inherit'},
				ti;

				if ( _.indexOf(extraArgs, "--build-only") > 0 || _.indexOf(extraArgs, "--ti") > 0){
					grunt.log.ok("build-only");
					grunt.log.ok("ti");
					extraArgs = _.drop(extraArgs); // remove the extra ti command
					ti = spawn("ti", extraArgs, tiOpts);
				}
				else {
					ti = spawn(getTitaniumPath(preferGlobal), extraArgs, tiOpts);
				}
...
function getTitaniumPath(preferGlobal) {
	...
	// if build-only is an option then use the ti version instead so we don't need to login to appc platform
	return 'appc';
	// } else {
		// return process.env.GRUNT_TITANIUM_TEST ?
			// path.resolve('node_modules', '.bin', 'appc ti') :
			// path.resolve('node_modules', 'grunt-titanium', 'node_modules', 'node-appc', 'lib', 'appc ti');
	// }
}
